...
const add = async ({ params, customer }, res) => {
  ...
  const updatedCustomer = Customer.findByIdAndUpdate({ _id: params.id }, { role: "admin" },
    {
      new: true,
      runValidators: true,
    }
...
const updateName = async ({ body, customer }, res) => {
  const data = await Customer.findOneAndUpdate({ _id: customer.customerId }, { name: body.name }, {
    new: true,
    runValidators: true,
  });
...
const checkEmailExist = async (req, res) => {
  ...
  // check email and send valid feedback
